...
def preceding_volatility(spot_series: pd.DataFrame, date: str, anchor_ts: pd.Timestamp) -> float:
    """Sample stdev of consecutive log returns over the VOL_WINDOW_MINUTES
    before the anchor (causal only -- nothing at/after anchor_ts is used).
    NaN if fewer than MIN_VOL_OBSERVATIONS snapshots fall in the window."""
    day_spots = spot_series[(spot_series["date"] == date) & spot_series["regular_hours"]]
    day_spots = day_spots.dropna(subset=["underlying_price"])
...
def add_baseline_features(outcome_dataset: pd.DataFrame, spot_series: pd.DataFrame) -> pd.DataFrame:
    ...
    for (date, ts), row in unique_anchors.iterrows():
        ...
        vol30[(date, ts)] = preceding_volatility(spot_series, date, ts)
        tod[(date, ts)] = minutes_since_open(ts)

    df["prev_5min_return"] = df.apply(lambda r: prev_ret[(r["date"], r["anchor_ts_et"])], axis=1)
    df["vol_30min"] = df.apply(lambda r: vol30[(r["date"], r["anchor_ts_et"])], axis=1)
