...
| `intraday/prices.json` | Every 10s | Always written |
...
Source: `intraday/prices.json`, polled every 10 seconds.
...
**Expiration settlement:** positions held past their expiration date always settle at $0 (`settleExpired()`), regardless of moneyness. This book has no margin to exercise or take assignment, so there's no realistic way to capture intrinsic value on an ITM position held to expiration — the honest simulation of "held it, didn't close it" is that it expires worthless, same as an OTM position. To realize intrinsic value, the position must be closed before expiration, same constraint a real no-exercise-capacity account would face.
